...
Security patches and vulnerability remediations are delivered exclusively to active release lines.

| Release Line | Status | Support Level |
| ------------ | ------ | ------------- |
| `0.4.x`      | Upcoming / current development target | Full support upon `0.4.0` publication |
| `0.3.x`      | Current published release line | Active maintenance (security fixes until `0.4.0` ships) |
| `< 0.3.0`    | Previous releases | End of Life (unsupported) |

*Lifecycle Policy*: `0.3.x` is the currently published release receiving critical security fixes. Once `0.4.0` is published, `0.4.x` becomes the active supported release line and `0.3.x` transitions to End of Life (EOL).
...
4. **Release & Advisory**:
   ...
   - A public GitHub Security Advisory (GHSA) will be published through GitHub's coordinated disclosure flow.
   - A CVE identifier may be requested or assigned by GitHub when applicable.
...
5. **Cryptographic Provenance**: Starting with the `v0.4+` release workflow, published packages include verifiable SLSA provenance attestations signed by npm/Sigstore (`npm audit signatures`). The first controlled OIDC publication with provenance is scheduled to be validated under Issue #41.
6. **Software Bill of Materials (SBOM)**: Starting with the `v0.4+` release pipeline, every release generates canonical SPDX 2.3 and CycloneDX 1.5 SBOMs matching the exact packed `.tgz` artifact.
