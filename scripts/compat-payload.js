...
export function buildScenarioCycle(length = 320) {
  const open = [];
  const high = [];
  const low = [];
  const close = [];
...
export function buildScenarioTrend(length = 320) {
  const open = [];
  const high = [];
  const low = [];
  const close = [];
...
export function buildScenarioChop(length = 320) {
  const open = [];
  const high = [];
  const low = [];
  const close = [];
...
export function buildScenarioVolatile(length = 320) {
  const open = [];
  const high = [];
  const low = [];
  const close = [];
...
export function computeCompatIndicators(inp) {
  return {
...
export function buildCompatPayload(policyOverride) {
  const policyPath = resolve(process.cwd(), "scripts/compat-policy.json");
  const compatPolicy = policyOverride || JSON.parse(readFileSync(policyPath, "utf8"));
  ...
  for (const [name, data] of Object.entries(scenarios)) {
    results[name] = {
      input: data,
      ours: computeCompatIndicators(data)
    };
  }
