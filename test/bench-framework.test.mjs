...
test("regression evaluator: evaluateScalingRatio (10k vs 100k)", () => {
  ...
  // Acceptable upper bound (e.g. 25x <= 35x) => PASS
  const bounded = evaluateScalingRatio({ medianMs: 0.10 }, { medianMs: 2.50 });
  assert.equal(bounded.status, "PASS");
  assert.equal(bounded.ratio, 25);

  // Quadratic regression (~100x time) => FAIL
  const quadratic = evaluateScalingRatio({ medianMs: 0.10 }, { medianMs: 8.00 });
  assert.equal(quadratic.status, "FAIL");
  assert.equal(quadratic.ratio, 80);
