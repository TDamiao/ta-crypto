...
7. **GitHub Release Asset & Canonical Registry Invariant**: `npmjs.org` is the single authoritative package registry for `ta-crypto`. Every production GitHub Release must permanently contain the exact validated `.tgz` tarball plus canonical SPDX 2.3 and CycloneDX 1.5 SBOMs. Actions artifacts serve as temporary execution evidence; GitHub Release assets serve as permanent release evidence.
